...
//есть ли запрос
if ($_SERVER['REQUEST_METHOD'] = 'POST' and isset($_POST['lot'])) {
    $lot = $_POST['lot'];
    ...
    if ($lot['category'] == 'Выберите категорию') { $errors['category'] = 'Не выбрана категория';};


    //проверяем изображение
    if (isset($_FILES['lot-image']['name']) and ($_FILES['lot-image']['tmp_name']<>"")) {
    ...
    if (count($errors))
    {
        ...
        //если нет ошибок
        //получаем ID категории лота

        $sql = 'SELECT id FROM categories WHERE name = `' . $lot['category'] . '` limit 1';
        $result = mysqli_query($link, $sql);
        if ($result) {
            $lot['category_id'] = mysqli_fetch_all($result, MYSQLI_ASSOC);
        } else {
            $error = mysqli_error($link);
        ...
        $stmt = db_get_prepare_stmt($link, $sql, [$lot['path'], $lot['title'], $lot['description'], $lot['start_price'], $lot['date_end'],
            $lot['bid_step'], $lot['category_id'] ?? '1']);
