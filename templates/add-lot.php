...
<form class="form form--add-lot container <?=$classname?>" action="add.php" method="post" enctype="multipart/form-data">
        ...
        <div class="form__item">
          ...
          <select id="category" name="lot[category]" required><!--required-->
            <option>Выберите категорию</option>
              <?php
              foreach ($categories as $value): ?>
              <?php $attrib_name = ($lot['category'] == $value['name']) ? "selected" : "";?>
              <option <?=$attrib_name?>><?= $value['name']?></option>
              <?php endforeach; ?>
          </select>
    ...
    $value = isset($lot['description']) ? $lot['description'] : ""; ?>
      <div class="form__item form__item--wide <?=$classname?>">
        ...
        <textarea id="message" name="lot[description]" placeholder="Напишите описание лота"><?=$value?></textarea><!--required-->
    ...
    <?php $classname = isset($lot['path']) ? "form__item--uploaded " : "";?>
      ...
      <div class="form__container-three">
        ...
        <div class="form__item form__item--small <?=$classname?>">
          ...
          <input id="lot-rate" type="number" name="lot[start_price]" placeholder="0"  value="<?= $value?>" required><!--required-->
        ...
        <div class="form__item form__item--small <?=$classname?>">
          ...
          <input id="lot-step" type="number" name="lot[bid_step]" placeholder="0"  value="<?= $value?>" required><!--required-->
